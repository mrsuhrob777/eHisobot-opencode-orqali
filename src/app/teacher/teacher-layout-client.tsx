          ...
          <div className="mb-2 px-4 py-2">
            <div className="relative" data-lang="true">
              <button onClick={() => setLangOpen(!langOpen)}
                className="w-full flex items-center justify-between gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-medium text-gray-700 shadow-sm outline-none transition-all cursor-pointer hover:border-emerald-300">
                <span>{lang === "uz" ? <>{langBadge("uz")} O'zbek</> : lang === "en" ? <>{langBadge("en")} English</> : <>{langBadge("ru")} Русский</>}</span>
                <svg className="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {langOpen && (
                <div className="absolute bottom-full left-0 right-0 mb-1 z-50 rounded-lg border border-gray-200 bg-white shadow-lg overflow-hidden">
                  <button onClick={() => { switchLang("uz"); setLangOpen(false); }}
                    className={`w-full flex items-center gap-2 px-3 py-2 text-xs font-medium transition-colors ${lang === "uz" ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50'}`}>
                    {langBadge("uz")} O'zbek
                  </button>
                  <button onClick={() => { switchLang("en"); setLangOpen(false); }}
                    className={`w-full flex items-center gap-2 px-3 py-2 text-xs font-medium transition-colors ${lang === "en" ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50'}`}>
                    {langBadge("en")} English
                  </button>
                  <button onClick={() => { switchLang("ru"); setLangOpen(false); }}
                    className={`w-full flex items-center gap-2 px-3 py-2 text-xs font-medium transition-colors ${lang === "ru" ? 'bg-emerald-50 text-emerald-700' : 'text-gray-700 hover:bg-gray-50'}`}>
                    {langBadge("ru")} Русский
                  </button>
                </div>
              )}
            </div>
        ...
        <header className="flex h-14 lg:h-16 items-center justify-between border-b border-gray-200 bg-white px-4 lg:px-6">
          ...
          <div className="flex items-center gap-2 lg:gap-3">
            ...
            <div className="hidden sm:block text-sm">
              ...
              <p className="text-xs text-gray-500">{user ? t(`role.${user.role}`, lang) : "teacher"}</p>
            </div>
          </div>
        </header>
